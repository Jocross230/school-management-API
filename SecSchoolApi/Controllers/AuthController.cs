...
namespace SecSchoolApi.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        private (string Token, DateTime ExpiresAt) GenerateJwtToken(ApplicationUser user, IList<string> roles)
        {
            ...
            var secretRaw = jwtSettings["Secret"] ?? throw new InvalidOperationException("JWT secret not configured");
            ...
            if (secretKeyBytes.Length * 8 < 256)
                throw new InvalidOperationException("JWT secret is too short for HS256. Provide at least a 256-bit key (32 bytes). Prefer a Base64-encoded 256+ bit key.");
